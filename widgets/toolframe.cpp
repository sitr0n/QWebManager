    ...
    //m_wrapper(new QWidget(this))
{
    ...
    this->layout()->addWidget(m_sidebar);
...
void ToolFrame::useEventlog()
{
    ...
    this->layout()->addWidget(m_eventlog);
...
void ToolFrame::putSettings(QWidget *w, QString context)
{
    w->setSizePolicy(QSizePolicy::MinimumExpanding, QSizePolicy::MinimumExpanding);
    ...
    QString application_name = context; // If it won't save settings, strip the string for white spaces
...
void ToolFrame::load_settings()
{
    ...
    emit resize(); // Remove this when settings are fixed
}
